const handleClickChooseMovie = async () => {
  try {
    setLoading()
    const { data } = await axios.get(`https://api.themoviedb.org/3/search/movie?language=pt-BR&page=${parseInt((Math.random() * 10) + 1)}&query=${randomWords()}&api_key=4227e6de25d38d75fffe9086cac8c78c&include_adult=false`)
